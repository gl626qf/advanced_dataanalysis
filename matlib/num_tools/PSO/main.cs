...
public class main{
    ...
    public static double negativeGauss(vector x){
		double sigma = 1;
		double mu = 0;
        // double K = 1 / (Sqrt(2*PI) * sigma) * Exp(-Pow(x-mu,2) / (2 * Pow(sigma,2)));
        double K = 1 / (Sqrt(2*PI) * sigma);
        double g_ = Exp(-Pow(x[0]-mu,2) / (2 * Pow(sigma,2))) + Exp(-Pow(x[1]-mu,2) / (2 * Pow(sigma,2)));
        // WriteLine("Gaussian PDF");
        // WriteLine($"Standard deviation, σ = {sigma}");
        // WriteLine($"Average value, µ = {mu}");
        return -K * g_;
    ...
    public static void Main(string[] args){
		...
		foreach(var arg in args){
			...
			if(arg == "-function1"){
				...
				// Run algorithm 
				vector solution = svarm.run(function1, a, b, seconds: 8);
			...
			if(arg == "-gauss"){
				...
				// // Initial vectors
				vector a = new vector(80,-60); 
				vector b = new vector(10,40); 

				// // Run algorithm 
				vector solution = svarm.run(negativeGauss, a, b, seconds: 2);

				solution.print("Solution: ");
				// vector solution = vector(2);
				Console.WriteLine("Function evaluated at vector: " + -negativeGauss(solution));
